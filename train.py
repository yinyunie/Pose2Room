...
def run(cfg):
    ...
    for name, module in net.module.named_children():
        cfg.log_string('--- module name: {0:s}: {1:d}.'.format(name, sum(p.numel() for p in module.parameters())))

    train(cfg=cfg, trainer=trainer, scheduler=scheduler, checkpoint=checkpoint, train_loader=train_loader, val_loader=val_loader)
